...
export default function DayPage({ log }) {
    return (
      <div className="p-6 bg-gray-800 shadow-lg rounded-lg max-w-7xl mx-auto transform hover:scale-105 transition-transform duration-300 ease-out">
        ...
        <div className="text-sm font-sans text-gray-300 mb-4">
          {log.activities.split('\n').map((line, index) => {
            const formattedLine = line
              .replace("Morgen", "<span class='text-lg text-orange-300'>Morgen</span>")
              .replace("Frokost", "<span class='text-lg text-orange-300'>Frokost</span>")
              .replace("Eftermiddag", "<span class='text-lg text-orange-300'>Eftermiddag</span>")
              .replace("Slutning af dagen", "<span class='text-lg text-orange-300'>Slutning af dagen</span>");
        ...
        <p className="text-lg font-sans text-blue-400">Team Members: {log.teamMembers}</p>
      </div>
    );
  }
